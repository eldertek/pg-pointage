...
tests:
	...
	@if [ "$(filter core,$(MAKECMDGOALS))" = "core" ]; then \
		$(VENV_ACTIVATE) && cd backend && $(PYTHON) manage.py test core.tests; \
	elif [ "$(filter users,$(MAKECMDGOALS))" = "users" ]; then \
		$(VENV_ACTIVATE) && cd backend && $(PYTHON) manage.py test users.tests; \
	elif [ "$(filter timesheets,$(MAKECMDGOALS))" = "timesheets" ]; then \
		$(VENV_ACTIVATE) && cd backend && $(PYTHON) manage.py test timesheets.tests; \
	elif [ "$(filter organizations,$(MAKECMDGOALS))" = "organizations" ]; then \
		$(VENV_ACTIVATE) && cd backend && $(PYTHON) manage.py test organizations.tests; \
	elif [ "$(filter sites,$(MAKECMDGOALS))" = "sites" ]; then \
		$(VENV_ACTIVATE) && cd backend && $(PYTHON) manage.py test sites.tests; \
	elif [ "$(filter alerts,$(MAKECMDGOALS))" = "alerts" ]; then \
		$(VENV_ACTIVATE) && cd backend && $(PYTHON) manage.py test alerts.tests; \
	elif [ "$(filter reports,$(MAKECMDGOALS))" = "reports" ]; then \
		$(VENV_ACTIVATE) && cd backend && $(PYTHON) manage.py test reports.tests; \
	else \
