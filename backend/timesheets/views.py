                            ...
                            if entry_type == Timesheet.EntryType.ARRIVAL:
                                ...
                                if schedule_detail.start_time_1 and schedule_detail.end_time_1:
                                    if schedule_detail.start_time_1 <= current_time <= schedule_detail.end_time_1:
                                        ...
                                        # Vérifier si le départ est avant la fin du créneau
                                        if current_time < relevant_end_time:
                                            is_matching = True
        ...
        try:
            ...
            for (employee_id, site_id, date), day_timesheets in groupby(sorted_timesheets, get_date_key):
                ...
                for arrival in arrivals:
                    ...
                    logger.debug(f"Vérification de retard pour {employee.get_full_name()} - timesheet.id={arrival.id}, is_late={arrival.is_late}, late_minutes={arrival.late_minutes}")
                    if arrival.is_late and arrival.late_minutes > 0:
                        ...
                        if arrival.late_minutes <= late_margin:
                            ...
                            logger.info(f"Retard dans la marge de tolérance: {arrival.late_minutes} minutes (marge: {late_margin} minutes)")
